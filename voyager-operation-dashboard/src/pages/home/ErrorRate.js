...
function ErrorRate({ startDates, finishedDates }) {
  ...
  function compareRuns(startDates, finishedDates, bin) {
    ...
    for (let i in finishedDates) {
      if (startObj[finishedDates[i][1]] !== null) {
        if (bin === "1 month" || bin === "3 months" || bin === "6 months") {
          ...
          diffArr = [
            ...diffArr,
            dateDiff(startObj[finishedDates[i][1]], finishedDates[i][0], "day"),
          ];
        } else {
    ...
    return (
      (binDates(diffArr, bin) / (startDates.length + binDates(diffArr, bin))) *
      100
    ).toFixed(2);
  }
  ...
  useEffect(() => {
    setBinDate("1 week");
    setErrorRate(compareRuns(startDates, finishedDates, "1 week"));
  }, []);
